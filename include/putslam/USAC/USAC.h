...
class USAC
{
	...
	protected:
		...
		virtual unsigned int 	generateMinimalSampleModels() = 0;
		virtual bool		    generateRefinedModel(std::vector<unsigned int>& sample, const unsigned int numPoints, 
														 bool weighted, double* weights) = 0;
		virtual bool		    validateSample() = 0;
		virtual bool		    validateModel(unsigned int modelIndex) = 0;
		virtual bool		    evaluateModel(unsigned int modelIndex, unsigned int* numInliers, unsigned int* numPointsTested) = 0;
		virtual void		    testSolutionDegeneracy(bool* degenerateModel, bool* upgradeModel) = 0;
		virtual unsigned int 	upgradeDegenerateModel() = 0;
		virtual void		    findWeights(unsigned int modelIndex, const std::vector<unsigned int>& inliers, 
											    unsigned int numInliers, double* weights) = 0;
		virtual void		    storeModel(unsigned int modelIndex, unsigned int numInliers) = 0;
