...
# The conversion is needed
	...
	p3 = subprocess.Popen("cut -d '_' -f 5" , stdin=p2.stdout, stdout=subprocess.PIPE, shell=True);
	p32 = subprocess.Popen("sort -n" , stdin=p3.stdout, stdout=subprocess.PIPE, shell=True);
	p4 = subprocess.Popen("cut -d '.' -f 1 > "+ dataset+"/matched" , stdin=p32.stdout, stdout=subprocess.PIPE, shell=True);
	p4.communicate();
	...
	for number in x.split('\n'):
		...
		# Copying images
		print("cp " + dataset+"/scene_"+str(number.rstrip())+".png " + dataset+"/rgb_%0.5d.png" % (i));		
		call("cp " + dataset+"/scene_"+str(number.rstrip())+".png " + dataset+"/rgb_%0.5d.png" % (i), shell=True);
		call("python "+ dataset + "/convertICLdepthToDepthImage.py " + dataset+"/scene_"+str(number.rstrip())+".depth " + dataset+"/depth_%0.5d.png" % (i), shell=True);
