...
std::vector<Eigen::Vector3f> RGBD::keypoints2Dto3D(
		std::vector<cv::KeyPoint> features, cv::Mat depthImage) {
...
std::vector<Eigen::Vector3f> RGBD::keypoints2Dto3D(
	...
	// Lets create 3D points
	std::vector<Eigen::Vector3f> features3D(features.size());
	int i = 0;
	for (std::vector<cv::KeyPoint>::iterator it = features.begin(); it!=features.end(); ++it)
	{
		// Feature are extracted with subpixel precision, so find closest pixel
		int uRounded = roundSize(it->pt.x, depthImage.cols);
		int vRounded = roundSize(it->pt.y, depthImage.rows);
		...
		float u = (it->pt.x - cameraMatrix.at<float>(0, 2)) / cameraMatrix.at<float>(0, 0);
		float v = (it->pt.y - cameraMatrix.at<float>(1, 2)) / cameraMatrix.at<float>(1, 1);
...
std::vector<cv::Point2f> RGBD::removeImageDistortion(
	...
	for (int i = 0; i < pointsUndistorted.rows; i++) {
		float x = pointsUndistorted.at<cv::Vec2f>(i)[0]
				* cameraMatrix.at<float>(0, 0) + cameraMatrix.at<float>(0, 2);
		float y = pointsUndistorted.at<cv::Vec2f>(i)[1]
				* cameraMatrix.at<float>(1, 1) + cameraMatrix.at<float>(1, 2);
		returnVector.push_back(cv::Point2f(x, y));
	}
	return returnVector;
}
